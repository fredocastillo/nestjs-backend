...
import { IHelperStringCurrencyOptions, IHelperStringPasswordOptions } from '../interfaces/helper.interface';
...
export class HelperStringService implements IHelperStringService {
  ...
  random(length: number): string {
    return faker.string.alphanumeric({ length: { min: length, max: length } });
  }
