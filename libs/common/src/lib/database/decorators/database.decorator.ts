...
import { InjectConnection, InjectModel, Prop, PropOptions, Schema, SchemaFactory, SchemaOptions } from '@nestjs/mongoose';
...
export function DatabaseSchema<T = any, N = MongooseSchema<T>>(entity: Type<T>): N {
  return SchemaFactory.createForClass<T>(entity) as N;
}

export function DatabaseQueryIn<T = string>(field: string, values: T[]): Record<string, any> {
  return {
    [field]: {
      $in: values,
    },
...
export function DatabaseQueryNin<T = string>(field: string, values: T[]): Record<string, any> {
  return {
    [field]: {
      $nin: values,
    },
...
export function DatabaseQueryEqual<T = string>(field: string, value: T): Record<string, any> {
  return {
    [field]: value,
  };
}

export function DatabaseQueryNotEqual<T = string>(field: string, value: T): Record<string, any> {
  return {
    [field]: {
      $ne: value,
    },
...
export function DatabaseQueryContain(field: string, value: string, options?: IDatabaseQueryContainOptions) {
  if (options?.fullWord) {
    return {
      [field]: {
