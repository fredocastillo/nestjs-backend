...
import { IHelperDateSetTimeOptions, IHelperDateFormatOptions, IHelperDateDiffOptions, IHelperDateCreateOptions, IHelperDateForwardOptions, IHelperDateBackwardOptions, IHelperDateRoundDownOptions } from '../interfaces/helper.interface';
import { ENUM_HELPER_DATE_DIFF, ENUM_HELPER_DATE_FORMAT } from '../enums/helper.enum';
...
export class HelperDateService implements IHelperDateService {
  ...
  constructor(private readonly configService: ConfigService) {
    this.defTz = this.configService.get<string>('app.timezone') ?? 'America/Chicago';
  }
  ...
  checkIso(date: string | Date | number): boolean {
    return moment(date, moment.ISO_8601, true).tz(this.defTz).isValid();
  }
  ...
  format(date: Date, options?: IHelperDateFormatOptions): string {
    ...
    return moment(date).tz(this.defTz).format(options?.format ?? ENUM_HELPER_DATE_FORMAT.DATE);
  }
  ...
  setTime(date: Date, { hour, minute, second, millisecond }: IHelperDateSetTimeOptions): Date {
    return moment(date).tz(this.defTz).set({
      hour: hour,
      minute: minute,
      second: second,
  ...
  addTime(date: Date, { hour, minute, second, millisecond }: IHelperDateSetTimeOptions): Date {
    return moment(date).tz(this.defTz).add({
      hour: hour,
      minute: minute,
      second: second,
      millisecond: millisecond,
  ...
  subtractTime(date: Date, { hour, minute, second }: IHelperDateSetTimeOptions): Date {
    return moment(date).tz(this.defTz).subtract({
      hour: hour,
      minute: minute,
      second: second,
    })
