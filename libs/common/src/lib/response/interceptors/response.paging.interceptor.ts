...
import { ResponsePagingDto, ResponsePagingMetadataDto } from '../dtos/response.paging.dto';
...
{
  constructor(private readonly reflector: Reflector, private readonly messageService: MessageService, private readonly configService: ConfigService, private readonly helperDateService: HelperDateService) {}
    ...
    if (context.getType() === 'http') {
      return next.handle().pipe(
        map(async (res: Promise<IResponsePaging<any>>) => {
          ...
          const xLanguage: string = request.__language ?? this.messageService.getLanguage();
          ...
          const message: string = this.messageService.setMessage( messagePath, { customLanguage: xLanguage, properties: messageProperties });
