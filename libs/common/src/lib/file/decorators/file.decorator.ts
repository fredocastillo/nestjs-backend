...
import { IFileUploadMultiple, IFileUploadMultipleField, IFileUploadMultipleFieldOptions, IFileUploadSingle } from '../interfaces/file.interface';
...
export function FileUploadMultipleFields(fields: IFileUploadMultipleField[], options?: IFileUploadMultipleFieldOptions): MethodDecorator {
  return applyDecorators(
    UseInterceptors(
...
export const FilePartNumber: () => ParameterDecorator = createParamDecorator(
  (_: unknown, ctx: ExecutionContext): number | undefined => {
    ...
    return headers['x-part-number'] ? Number(headers['x-part-number']) : undefined;
  }
);
